...
/** Alerts API */
export async function pushAlert(code, toUid, message, type = 'info') {
  const col = collection(db, 'sessions', code, 'alerts');
  await addDoc(col, { toUid, type, message, createdAt: serverTimestamp() });
}

export function listenToAlerts(code, playerUid, onAlert) {
  const col = collection(db, 'sessions', code, 'alerts');
  const q = query(col, where('toUid', 'in', [playerUid, '*']), orderBy('createdAt', 'asc'));
